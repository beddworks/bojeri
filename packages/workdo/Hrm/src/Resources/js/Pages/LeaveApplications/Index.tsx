...
export default function Index() {
    ...
    const getStatusBadge = (status: string) => {
        ...
        const statusClass = statusColors[status as keyof typeof statusColors] || 'bg-gray-100 text-gray-800';
        const statusText = t(status?.charAt(0).toUpperCase() + status?.slice(1) || 'Unknown');
        return `<span class="px-2 py-1 rounded-full text-sm font-medium ${statusClass}">${statusText}</span>`;
    };
    ...
    const tableColumns = [
        ...
        },
        {
            ...
            render: (value: string, row: any) => {
                return (
                    <span className={`px-2 py-1 rounded-full text-sm font-medium ${row.leave_type?.is_paid
        ...
        },
         {
            ...
            header: t('Status'),
            sortable: false,
            render: (value: string) => <span dangerouslySetInnerHTML={{ __html: getStatusBadge(value) }} />
        },
                            ...
                            <Tooltip delayDuration={0}>
                                <TooltipTrigger asChild>
                                    <Button variant="ghost" size="sm" asChild className="h-8 w-8 p-0 text-orange-600 hover:text-orange-700">
                                        <a href={getImagePath(leaveapplication.attachment)} target="_blank" rel="noopener noreferrer">
                                ...
                                <DataTable
                                    ...
                                    emptyState={
                                        <NoRecordsFound
                                            ...
                                            description={t('Get started by creating your first Leave Application.')}
                                            ...
                                            createButtonText={t('Create Leave Application')}
                                            className="h-auto"
                                        />
                                    }
                                />
                                        ...
                                        <Card key={leaveapplication.id} className="p-0 hover:shadow-lg transition-all duration-200 relative overflow-hidden flex flex-col h-full min-w-0">
                                            ...
                                            <div className="p-4 bg-gradient-to-r from-primary/5 to-transparent border-b flex-shrink-0">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <div className="min-w-0 flex-1">
                                                        <h3 className="font-semibold text-sm text-gray-900">{leaveapplication.employee?.name || '-'}</h3>
                                                    </div>
                                                </div>
                                            </div>
                                            ...
                                            <div className="p-4 flex-1 min-h-0">
                                                ...
                                                <div className="grid grid-cols-2 gap-4">
                                                    <div className="text-xs min-w-0">
                                                        <p className="text-muted-foreground mb-1 text-xs uppercase tracking-wide">{t('Applied On')}</p>
                                                        <p className="font-medium text-xs">{leaveapplication.created_at ? formatDate(leaveapplication.created_at) : '-'}</p>
                                                    </div>
                                                    <div className="text-xs min-w-0">
