...
export default function Create() {
    ...
    const customFields = useFormFields('getCustomFields', { ...data, module: 'Hrm', sub_module: 'Employee' }, setData, errors, 'create', t);
                            ...
                            <TabsContent value="employment" className="space-y-6 mt-6">
                                <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
                                    <div>
                                        ...
                                        {users?.length === 0 && auth?.user?.permissions?.includes('create-users') && (
                                            <p className="text-xs text-gray-500 mt-1">
                                                {t('Create user here.')} <button type="button" onClick={(e) => { e.preventDefault(); router.get(route('users.index')); }} className="text-blue-600 hover:underline">{t('Create user')}</button>
                                            </p>
                                        )}
                                    ...
                                    <div>
                                        ...
                                        {filteredBranches?.length === 0 && auth?.user?.permissions?.includes('create-branches') && (
                                            <p className="text-xs text-gray-500 mt-1">
