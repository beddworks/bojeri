...
export default function Index() {
    ...
    const dropboxBtn = usePageButtons('dropboxBtn', { module: 'Vender', settingKey: 'Dropbox Vender' });
                    ...
                    {vendor.user?.is_disable === 1 ? (
                        ...
                        <TooltipProvider>
                            {auth.user?.permissions?.includes('view-vendor-detail-report') && (
                                <Tooltip delayDuration={0}>
                                    <TooltipTrigger asChild>
                                        <Button variant="ghost" size="sm" onClick={() => router.visit(route('account.reports.vendor-detail', vendor.user_id))} className="h-8 w-8 p-0 text-orange-600 hover:text-orange-700">
