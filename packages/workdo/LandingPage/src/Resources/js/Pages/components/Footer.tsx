        ...
        return (
            <div className={config.layout === 'split' ? 'space-y-6' : ''}>
                <Link href={route('landing.page')} className={config.companyName} style={config.layout !== 'split' ? { color: colors.primary } : {}}>
                    {logoUrl ? (
                        <img src={logoUrl} alt={companyName} className={config.layout === 'centered' ? 'w-auto max-w-24 object-contain mx-auto' : 'w-auto max-w-24 object-contain'} />
                    ) : (
                        companyName
                    )}
                </Link>
                            ...
                            <div key={index} className="min-w-0">
                                ...
                                <ul className="space-y-2">
                                    {section.links.slice(0, 4).map((link: any, linkIndex: number) => (
                                        <li key={linkIndex}>
                                            {link.target === '_blank' ? (
                                                <a href={link.href?.startsWith('/page/') ? route('custom-page.show', link.href.replace('/page/', '')) : link.href} target="_blank" rel="noopener noreferrer" className="text-slate-700 hover:text-gray-900 transition-colors text-sm hover:underline">
                                                    {link.text}
                                                </a>
                                            ) : (
                                                <Link href={link.href?.startsWith('/page/') ? route('custom-page.show', link.href.replace('/page/', '')) : link.href} className="text-slate-700 hover:text-gray-900 transition-colors text-sm hover:underline">
                                                    {link.text}
                                                </Link>
                                            )}
                                        </li>
                            ...
                            <div key={index}>
                                ...
                                <ul className="space-y-3">
                                    {section.links.map((link: any, linkIndex: number) => (
                                        <li key={linkIndex}>
                                            {link.target === '_blank' ? (
                                                <a href={link.href?.startsWith('/page/') ? route('custom-page.show', link.href.replace('/page/', '')) : link.href} target="_blank" rel="noopener noreferrer" className="text-slate-700 hover:text-gray-900 transition-colors text-sm">
                                                    {link.text}
                                                </a>
                                            ) : (
                                                <Link href={link.href?.startsWith('/page/') ? route('custom-page.show', link.href.replace('/page/', '')) : link.href} className="text-slate-700 hover:text-gray-900 transition-colors text-sm">
                                                    {link.text}
                                                </Link>
                                            )}
                                        </li>
