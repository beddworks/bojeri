...
export function BrandProvider({ children }: { children: ReactNode }) {
  ...
  const getPrimaryColor = () => {
    return settings.themeColor === 'custom'
      ? settings.customColor || '#10b981'
      : themeColors[settings.themeColor as keyof typeof themeColors] || '#10b981';
  };
  ...
  const getLogoSrc = () => {
    ...
    return isDark ? (settings.logo_light || settings.logo_dark || '') : (settings.logo_dark || settings.logo_light || '');
  };
